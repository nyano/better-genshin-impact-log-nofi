...
# 1.1
ExitOKLog目录是JS脚本，可以放到BetterGI的JsScript并添加配置组。可以自定义添加日志，以便识别并添加提示。
脚本自带JS自定义通知，可以通过BetterGI自带的通知进行通知，也可以打开该软件添加关键字通知。JS自定义通知也可以在通用里面关掉。没做独立开关，我这2个都开着，以便2次提醒。

# 1.2 待办
添加个最后执行时间与当前时间超时对比提醒。因为我给了BGI的位置，然后发现软件的log位置没改，开了半天，笑死。
